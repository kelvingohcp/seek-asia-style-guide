// brand colors
export const saBlue = '#0D3880';

export const dbBlue = '#0C4B85';
export const dbHirer = '#0e7e8b';
export const dbCTA = '#FF9000';
export const jsBlue = '#1C3F94';
export const jsHirer = '#1B953E';
export const jsHirerGreen = jsHirer; // Alias
export const jsCTA = '#FFF200';

// Neutrals
export const saBlack = '#000000';
export const saGrey1 = '#333333';
export const saGrey2 = '#666666';
export const saGrey3 = '#999999';
export const saGrey4 = '#CCCCCC';
export const saGrey5 = '#EEEEEE';
export const saGrey6 = '#f8f8fa';
export const saWhite = '#FFFFFF';

// Blue shades added at 16 Aug 2018
// Please refer to https://brand.seekasia.com/product/color-usage/
export const saBlue1 = '#206E8F';
export const saBlue2 = '#298EB9';
export const saBlue3 = '#94C9E0';
export const saBlue4 = '#DDF1FA';
export const saBlue5 = '#EEF8FC';

export const saVisitedLink = '#6c488f';

// Action colors
export const actionYellow = jsCTA;
export const actionOrange = dbCTA;
export const actionBlue = saBlue2; // Alias, equivalent to "saBlue2"
export const actionGreen = '#63B209';
export const actionRed = '#FF4B4B';
export const actionPastelBlue = '#DDF1FA'; // Alias, equivalent to "saBlue4"
export const actionTurquoise = '#0E7E8B';

export const lightBlue = '#EEF8FC'; // Alias, equivalent to "saBlue5"

// Border width
export const borderWidthButton = '1px';
