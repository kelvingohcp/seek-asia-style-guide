...
const styleGuidePath = path.dirname(require.resolve('seek-asia-style-guide'));

const styleGuideModules = ['react', 'theme', 'fonts', 'jobStreet', 'jobsDB'];
...
const getIncludePaths = options => {
  ...
  return {
    relative: [
      ...styleGuideModules.map(module => `seek-asia-style-guide/${module}`),
      ...extraIncludePaths
    ],
    absolute: [
...
const warn = message =>
  console.warn(
    chalk.yellow(`\nSEEK ASIA STYLE GUIDE WARNING:\n${singleLine(message)}\n`)
  );

const error = message => {
  throw new Error(
    chalk.red(`\nSEEK ASIA STYLE GUIDE ERROR:\n${singleLine(message)}\n`)
  );
};
  ...
  config.module.rules.forEach(rules => {
    if (!rules.include) {
      error(
        ...
        "include" value: ${rules.test}. This is required by 'seek-asia-style-guide'
